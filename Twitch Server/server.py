...
def getGames():
    ...
    formatted = []
    for game in games:
        formatted.append(game[0])
    return formatted


def getClips(game, amount):
    clips = database.getGameClipsByStatusLimit(game, "DOWNLOADED", amount)
    files = []
    for clip in clips:
        info = {"id" : clip.id, "mp4" : clip.mp4, "streamer_name" : clip.streamer_name, "duration" : clip.vid_duration, "clip_title" : clip.clip_name}
        files.append(info)
    return files
    #print(files)

def getClipsWithoutIds(game, amount, ids):
    clips = database.getGameClipsByStatusWithoutIds(game, "DOWNLOADED", amount, ids)
    files = []
    for clip in clips:
        info = {"id" : clip.id, "mp4" : clip.mp4, "streamer_name" : clip.streamer_name, "duration" : clip.vid_duration, "clip_title" : clip.clip_name}
        files.append(info)
...
def VideoGeneratorRenderStatus():
    ...
    while True:
        ...
        try:
            ...
            music_cats = renderData["music"]
            with open(f"{current_path}/music-cats.save", 'wb') as pickle_file:
                pickle.dump(music_cats, pickle_file)
    ...
    def do_GET(self):
        ...
        try:
            if "/getgames" == self.path:
                ...
                music_cats = None
                if os.path.exists(f"{current_path}/usertable.save"):
                    with open(f'{current_path}/music-cats.save', 'rb') as pickle_file:
                        music_cats = pickle.load(pickle_file)
