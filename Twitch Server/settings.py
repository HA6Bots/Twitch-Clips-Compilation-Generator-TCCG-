...
def generateConfigFile():
    if not os.path.isfile(configpath):
        ...
        config.add_section("twitch_settings")
        config.set("twitch_settings", 'language', 'en')
        config.set("twitch_settings", 'client_id', '')
        config.set("twitch_settings", 'bearer', '')
...
def loadValues():
    global FTP_PORT, HTTP_PORT, serveraddress, videoGeneratorAddress, videoGeneratorFTPPort, videoGeneratorHTTPPort, videoGeneratorFTPUser,\
        videoGeneratorFTPPassword, language, databasehost, databasepassword, databaseuser, client_id, bearer
    config = configparser.RawConfigParser()
    ...
    language = config.get('twitch_settings', 'language')
    client_id = config.get('twitch_settings', 'client_id')
    bearer = config.get('twitch_settings', 'bearer')
